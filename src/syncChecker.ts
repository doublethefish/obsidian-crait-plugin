...
export default class SyncChecker {
  ...
  public deviceName(): string {
    return this.syncInstance.deviceName
      ? this.syncInstance.deviceName
      : this.syncInstance.getDefaultDeviceName();
  }
  ...
  public handleUnload(): void {
    ...
    // Unload the listener
    this.syncInstance._[syncEmitterName] = this.syncInstance._[syncEmitterName].filter(
      (listener: { ctx: string }) => {
