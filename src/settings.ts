...
export default class CronSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Run all jobs on Obsidian startup instead of waiting for the job intervals to pass"
      )
      .addToggle((toggle) =>
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Whether or not to wait for Obsidian sync before running any jobs globally."
      )
      .addToggle((toggle) =>
        toggle
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Whether or not to load jobs at all on Mobile devices. If disabled even jobs with mobile enabled will not run."
      )
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.enableMobile).onChange(async (value) => {
    ...
    new Setting(containerEl).setName("Inactivity Jobs").setDesc(desc);
    ...
    this.plugin.settings.jobs.forEach((craitJob, index) => {
      const jobSetting = new Setting(this.containerEl)
        ...
        .addExtraButton((button) => {
          button
            .setIcon(
              craitJob.settings.enableMobile ? "lucide-phone" : "lucide-phone-off"
            )
            .setTooltip("Toggle job on mobile")
            .onClick(async () => {
              this.plugin.settings.jobs[index].settings.enableMobile =
                !craitJob.settings.enableMobile;
              await this.plugin.saveSettings();
              // refresh
              this.display();
            });
      ...
      jobSetting
        .addExtraButton((button) => {
          button
            .setIcon(
              craitJob.settings.disableSyncCheck ? "paused" : "lucide-check-circle-2"
            )
            .setTooltip(
              "Toggle Sync check for this job. Presently: " +
                (craitJob.settings.disableSyncCheck ? "disabled" : "enabled")
            )
            .onClick(() => {
              this.plugin.settings.jobs[index].settings.disableSyncCheck =
                !craitJob.settings.disableSyncCheck;
              this.plugin.saveSettings();
              // Force refresh
              this.display();
            });
